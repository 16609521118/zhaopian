...
- **全屏查看**：左右滑动翻页、双指缩放、双击放大/还原；底部「详情 + 分享」（详情含文件名/尺寸/拍摄时间/大小）
- **视频查看**：我的图片、文件夹映射中的视频（mp4/mov/m4v/3gp）显示缩略图与播放角标，点击 AVPlayer 全屏播放
- **我的图片**：从文件夹（系统文件选择器）导入图片/视频到 App 沙盒（Documents/Images），网格浏览、全屏查看、长按删除
...
- **局域网映射（IP 访问）**：App 内启动局域网 HTTP 服务，显示 `http://127.0.0.1:端口` 与手机局域网 IP；电脑浏览器访问即可浏览/下载/上传（PUT）「我的图片」文件夹——即“IP 映射”形态
...
│   ├── SharedViews.swift            # 权限引导 / 视频缩略图与播放 / 详情视图
│   └── Info.plist
