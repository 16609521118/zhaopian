...
final class FolderViewModel: ObservableObject {
    ...
    func mount(url: URL) -> String? {
        ...
        do {
            // iOS 上文件选择器返回的 URL 本身即 security-scoped，bookmark 无需 withSecurityScope 选项
    ...
    private func loadBookmarks() -> [String: Data] {
        guard let data = UserDefaults.standard.data(forKey: bookmarksKey),
              let dict = try? NSKeyedUnarchiver.unarchivedObject(ofClass: NSDictionary.self, from: data) as? [String: Data]
        else {
            return [:]
        }
        return dict
