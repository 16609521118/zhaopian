...
class ViewController: UIViewController, WKNavigationDelegate, WKUIDelegate {
    ...
    private enum PickerMode { case mount, importMedia }
    private var pickerMode: PickerMode = .mount
...
extension ViewController: WKScriptMessageHandler {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "mountFolder":
            pickerMode = .mount
            presentDocumentPicker(contentTypes: [.folder], multiple: false)
        case "importMedia":
            pickerMode = .importMedia
...
// MARK: - UIDocumentPicker 委托（选文件夹/选文件）
extension ViewController: UIDocumentPickerDelegate {
    ...
    private func mountFolder(from sourceURL: URL) {
        ...
        guard fm.fileExists(atPath: sourceURL.path, isDirectory: &isDir), isDir.boolValue else {
            evaluateJS("window.__nativeMountResult({\"items\":[]})")
            showToast("请选择一个文件夹")
            return
        }
        ...
        do {
            ...
            let payload: [String: Any] = ["folderId": folderId, "name": sourceURL.lastPathComponent, "items": items]
            evaluateJS("window.__nativeMountResult(\(Self.jsonString(payload)))")
        } catch {
            showToast("挂载失败：\(error.localizedDescription)")
            evaluateJS("window.__nativeMountResult({\"items\":[]})")
        }
    }
