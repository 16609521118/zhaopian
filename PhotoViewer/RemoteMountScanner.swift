...
class RemoteMountScanner {
    ...
    static func scan(baseURL: URL, completion: @escaping ([RemoteMediaItem]?) -> Void) {
        ...
        scanDirectory(url: baseURL, depth: 0, items: &items) { ok in
            if ok && !items.isEmpty {
                completion(items)
            } else if ok && items.isEmpty {
                completion([])   // 目录可访问但无媒体文件
            } else {
                completion(nil)  // 访问失败
            }
        }
    ...
    private static func scanDirectory(url: URL, depth: Int, items: inout [RemoteMediaItem], done: @escaping (Bool) -> Void) {
        if depth >= maxDepth {
            done(true)
            return
        }
        ...
        webdavList(url: url) { davResult in
            if let davItems = davResult {
                // WebDAV 成功
                collect(davItems, depth: depth, items: &items, done: done)
            } else {
                // 回退 HTML 目录列表
                htmlList(url: url) { htmlItems in
                    if let htmlItems = htmlItems {
                        collect(htmlItems, depth: depth, items: &items, done: done)
                    } else {
                        done(false)
                    }
                }
    ...
    private static func collect(_ found: [(name: String, url: URL, size: Int, isDir: Bool)], depth: Int, items: inout [RemoteMediaItem], done: @escaping (Bool) -> Void) {
        // 先收集本层媒体文件
        ...
        var success = true
        var localItems: [RemoteMediaItem] = []
        let lock = NSLock()
        for d in dirs {
            group.enter()
            var subItems: [RemoteMediaItem] = []
            scanDirectory(url: d.url, depth: depth + 1, items: &subItems) { ok in
                lock.lock()
                if !ok { success = false }
                localItems.append(contentsOf: subItems)
                lock.unlock()
                group.leave()
            }
        }
    ...
    private static func parseWebDAV(_ xml: String, baseURL: URL) -> [(name: String, url: URL, size: Int, isDir: Bool)]? {
        ...
        for block in blocks {
            ...
            // 解析为 URL
            var itemURL: URL?
            if hrefValue.hasPrefix("http") {
                itemURL = URL(string: hrefValue)
            } else if hrefValue.hasPrefix("/") {
                itemURL = URL(string: hrefValue, relativeTo: baseURL)?.absoluteURL
            } else {
                itemURL = URL(string: hrefValue, relativeTo: baseURL)
            }
            guard let resolved = itemURL else { continue }
